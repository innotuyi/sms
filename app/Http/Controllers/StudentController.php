...
class StudentController extends SchoolBaseController
{
    ...
    {      
        ...
        $data['school_id'] = $this->getSchoolId(); // Add school_id to user data
        
        // Retrieve the school code
        $school = $this->my_school->find($this->getSchoolId());
        $schoolCode = strtoupper($school->school_code);
    
        // Generate a unique admission number using school code and a random unique ID
        $uniqueId = mt_rand(1000, 99999);
